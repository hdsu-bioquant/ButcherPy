...
def signature_heatmap(NMFobj, path, ranks = "all", top_n = 0):
        """
        ...
        if top_n == 0:
            ...
            sig_df = NMFobj.feature_contributions
            if not sig_df.empty:
        ...
        # In case the top_n parameter is greater than 0, the top_n genes are used
        elif top_n > 0 and top_n <= len(NMFobj.input_matrix["genes"]):
            if top_n%1!=0:
                top_n = int(top_n)+1
            else:
                top_n = int(top_n)
            ...
            available_ranks = []
            if ranks=="all":
                index = list(range(len(NMFobj.ranks)))
                available_ranks = NMFobj.ranks
            else:
            ...
            if len(index)!=0:
                Ws = [NMFobj.WMatrix[i] for i in index]
                Ws = np.hstack(Ws)
